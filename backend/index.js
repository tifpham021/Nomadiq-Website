...
app.post("/api/signup", async (req,res) => {
    ...
    try {
        await newUser.save();
        res.status(201).json({success: true, data:newUser, message: "Your account was successfully created!"});
    } catch (error) {
...
app.post("/api/resetting-pass-email", async (req,res) => {
    ...
    if (!email) {
        return res.status(400).json({success: false, message: "Please enter the email you registered with"});
    }

    try {
        const user = await User.findOne({email});
        if (!user) {
            return res.status(400).json({success: false, message: "This email isn't in our database"});
        }
